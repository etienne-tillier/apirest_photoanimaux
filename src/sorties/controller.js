...
const getSortieId = async (req,res) => {
    try {
        const sortie = await pool.query(queries.getSortieId, [
            req.params.id
        ])
        let especesForSortie = await pool.query(queries.getAllEspeceForASortie, [
            req.params.id
...
const updateSortie = async (req,res) => {
    try {
        const {idutilisateur, date, description, latitude, longitude, especes} = req.body
        ...
        await pool.query(queries.deleteAllEspeceForSortie, [
            req.params.id
        ])
